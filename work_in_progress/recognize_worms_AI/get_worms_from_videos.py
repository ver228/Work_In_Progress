...
#%%
def get_all_img_ROI_fixed(img, frame_data, roi_size):
    ...
    indexes = np.zeros(tot_worms, np.int)
    
    for ii, irow in enumerate(worms_in_frame):
    ...
    return worm_imgs, roi_corners, indexes
...
def read_group_images(fid, img_h5path, sample_traj_data, roi_size):
    ...
    for ii, (current_frame, frame_data) in enumerate(traj_group_by_frame):
        ...
        worms_roi, roi_corners, indexes = get_all_img_ROI_fixed(img, frame_data, roi_size)
