...
def _init_tables(skeletons_file, save_file, roi_size):
    with pd.HDFStore(skeletons_file, 'r') as fid:
        trajectories_data = fid['/trajectories_data']
    
    trajectories_data['original_table_row'] = trajectories_data.index
    
    tot_rows = trajectories_data.is_good_skel.sum() 
    trajectories_data = trajectories_data[trajectories_data.is_good_skel]
    
    assert len(trajectories_data) == tot_rows
...
def get_roi_skel_sample(skeletons_file, save_file, roi_size, chuck_size):
    #%%
    ...
    trajectories_data = _init_tables(skeletons_file, save_file, roi_size)
...
if __name__ == '__main__':
    ...
    save_file = base_name + '_sampleshort.hdf5'
    roi_size = 120
    chuck_size = 100
    
    get_roi_skel_sample(skeletons_file, save_file, roi_size, chuck_size)
