...
def _getFluoThreshold(pix_valid):
    MAX_PIX = 255
    thresh = _getBWThreshold(MAX_PIX - pix_valid)
    thresh = MAX_PIX - thresh

    # TODO: partition this into a separate function
...
def _getWormThreshold(pix_valid, worm_bw_thresh_factor, analysis_type):
    # caculate threshold
    if analysis_type == "WORM":
        if is_light_background:
            thresh = _getBWThreshold(pix_valid)
        else:
            thresh = _getFluoThreshold(pix_valid)
    ...
    with tables.File(masked_image_file, 'r') as mask_fid, \
        ...
        for frame_number in range(initial_frame, last_frame, buffer_size):
            ...
            for ROI_cnt in ROI_cnts:
                ...
                # caculate threshold
                thresh = _getWormThreshold(pix_valid, worm_bw_thresh_factor, analysis_type)
            ...
            if buffer_features:
                plate_worms.append(buffer_features)
