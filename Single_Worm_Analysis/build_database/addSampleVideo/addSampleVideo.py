...
def getSubSampledName(mask_file_name):
    return mask_file_name.replace('.hdf5', '_sample.mp4')

def createSmallVideo(mask_file_name, sample_video_name ='', time_factor = 8, 
                     size_factor = 5, expected_fps=30):
    #%%
    if not sample_video_name:
        sample_video_name = getSubSampledName(mask_file_name)
    
    # initialize timers
    base_name = mask_file_name.rpartition('.')[0].rpartition(os.sep)[-1]
...
if __name__ == '__main__':
    ...
    mask_file_name = '/Volumes/behavgenom_archive$/Avelino/Worm_Rig_Tests/Agar_Test/MaskedVideos/Agar_Screening_101116/unc-9_N3_F1-3_Set1_Pos3_Ch4_12112016_002739.hdf5'
    createSmallVideo(mask_file_name)
